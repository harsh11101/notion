...
export const TrashBox=()=>{
    ...
    if(documents===undefined){
        return(
            <div className="h-hull flex items-center justify-center p-4">
                <Spinner size="lg" />
            </div>
        )
    }
    ...
    return(
        <div className="text-sm">
            <div className="flex items-center gap-x-1 p-2">
                ...
                <Input value={search} onChange={(e)=>setSearch(e.target.value)} className="h-7 px-2 focus-visible:ring-transparent bg-secondary" placeholder="Filter by page" />
            </div>
            <div className="mt-2 px-1 pb-1">
                <p className="hidden last:block text-xs center text-muted-foreground mb-2">
                    No documents found
                </p>
                {filteredDocuments?.map((document)=>(
                    <div key={document._id} role="button" onClick={()=>onClick(document._id)} className="text-sm rounded-sm w-full hover:bg-primary/5 flex items-center text-primary justify-between">
                        ...
                        <div className="flex items-center">
                            <div role="button" onClick={(e)=>onRestore(e,document._id)} className="rounded-sm p-2 hover:bg-nuetral-200 dark:hover:bg-neutral-600">
